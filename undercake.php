 ...
 *
 * This adaptation for use in CakePHP made by Andrei Costin Balaianu
 *
 *  For now, in CakePHP we can only use this method of calling underscore.php methods:
 *
 *    ex: _uc( array(1, 2, 3) )->map(function($n) { return $n * 2; });
 *
 *  I will try to make this into a component at a later time
 *
 */

// Returns an instance of _uc for OO-style calls
function _uc($item=null) {
  $_uc = new _uc;
  if(func_num_args() > 0) $_uc->_wrapped = $item;
  return $_uc;
}
...
class _uc {
  ...
  public function chain($item=null) {
    ...
    $_uc = (isset($this) && isset($this->_chained) && $this->_chained) ? $this : _uc($item);
    $_uc->_chained = true;
    return $_uc;
  }
  ...
  public function reduceRight($collection=null, $iterator=null, $memo=null) {
    ...
    $_uc = new self;
    return self::_wrap($_uc->reduce($collection, $iterator, $memo));
  }
  ...
  public function invoke($collection=null, $function_name=null, $arguments=null) {
    ...
    $_uc = new self;
    list($collection, $function_name) = $_uc->first($args, 2);
    $arguments = $_uc->rest(func_get_args(), 2);
  ...
  public function any($collection=null, $iterator=null) {
    ...
    $_uc = new self;
    if(!is_null($iterator)) $collection = $_uc->map($collection, $iterator);
  ...
  public function initial($collection=null, $n=null) {
    ...
    $_uc = new self;
    return self::_wrap($_uc->first($collection, $first_index));
  }
  ...
  public function compact($collection=null) {
    ...
    $_uc = new self;
    return self::_wrap($_uc->select($collection, function($val) {
      return (bool) $val;
    }));
  }
  ...
  public function flatten($collection=null, $shallow=null) {
    ...
    if(count($collection) > 0) {
      foreach($collection as $item) {
        if(is_array($item)) {
          $_uc = new self;
          $return = array_merge($return, ($shallow) ? $item : $_uc->flatten($item));
        }
        else $return[] = $item;
      }
    }
  ...
  public function intersection($array=null) {
    ...
    $_uc = new self;
    $return = $_uc->first($arrays);
    foreach($_uc->rest($arrays) as $next) {
      if(!$_uc->isArray($next)) $next = str_split((string) $next);
  ...
  public function union($array=null) {
    ...
    $_uc = new self;
    return self::_wrap($_uc->flatten(array_values(array_unique(call_user_func_array('array_merge', $arrays)))));
  }
  ...
  public function lastIndexOf($collection=null, $item=null) {
    ...
    krsort($collection);
    $_uc = new self;
    return self::_wrap($_uc->indexOf($collection, $item));
  }
  ...
  public function range($stop=null) {
    ...
    $_uc = new self;
    $args = $_uc->reject($args, function($val) {
      return is_null($val);
    });
    ...
    if($step > 0 && $_uc->last($results) >= $stop) array_pop($results);
    elseif($step < 0 && $_uc->last($results) <= $stop) array_pop($results);
  ...
  public function zip($array=null) {
    ...
    $_uc = new self;
    $num_return_arrays = $_uc->max($_uc->map($arrays, function($array) {
      return count($array);
    }));
    $return_arrays = $_uc->range($num_return_arrays);
  ...
  public function max($collection=null, $iterator=null) {
    ...
    arsort($results);
    $_uc = new self;
    $first_key = $_uc->first(array_keys($results));
  ...
  public function extend($object=null) {
    ...
    $array = (array) $object;
    $_uc = new self;
    $extensions = $_uc->rest(func_get_args());
  ...
  public function isEqual($a=null, $b=null) {
    ...
    if(is_object($a) || is_array($a)) {
      ...
      $_uc = new self;
      $keys_equal = $_uc->isEqual($_uc->keys($a), $_uc->keys($b));
      $values_equal = $_uc->isEqual($_uc->values($a), $_uc->values($b));
